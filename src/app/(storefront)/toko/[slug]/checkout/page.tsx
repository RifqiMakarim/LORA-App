...
export default function DedicatedCheckoutPage() {
    ...
    // Handler Unduh Gambar QRIS
    const handleDownloadQris = async (e?: React.MouseEvent) => {
        if (e) e.stopPropagation();
        if (!businessInfo?.qris_image_url) return;
        try {
            const response = await fetch(businessInfo.qris_image_url);
            const blob = await response.blob();
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `QRIS-${(businessInfo.name || 'Toko').replace(/[^a-zA-Z0-9]/g, '-')}.png`;
            ...
            window.open(businessInfo.qris_image_url, '_blank');
        }
    };
    ...
    return (
        <div className="space-y-6 pb-20">
                ...
                <div className="lg:col-span-5 space-y-6 sticky top-24">
                    {selectedPaymentMethod === 'qris' && (
                        <div className="bg-white p-6 rounded-3xl border-2 border-terracotta/30 shadow-xl space-y-4 text-center">
                            ...
                            {/* Gambar QRIS Toko (Lebar & Adaptif dengan Border & Zoom) */}
                            {businessInfo?.qris_image_url ? (
                                <div className="space-y-3">
                                    <div
                                        onClick={() => setIsQrZoomOpen(true)}
                                        className="group relative bg-slate-50 p-2 sm:p-3 rounded-2xl border-2 border-slate-200 hover:border-terracotta/50 max-w-[360px] mx-auto flex flex-col items-center justify-center shadow-inner cursor-pointer transition-all hover:shadow-md"
                                        title="Klik untuk memperbesar gambar QRIS"
                                    >
                                        <div className="relative w-full bg-white p-2 rounded-xl border border-slate-200 shadow-xs overflow-hidden flex items-center justify-center">
                                            <img
                                                src={businessInfo.qris_image_url}
                                                alt="Kode QRIS Toko"
                                                className="w-full h-auto min-h-[220px] max-h-[380px] object-contain transition-transform duration-300 group-hover:scale-[1.02]"
                                            />

                                            {/* Hover Zoom Badge Overlay */}
                                            <div className="absolute inset-0 bg-slate-950/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5 text-white text-xs font-bold backdrop-blur-[2px] rounded-xl">
                                                <ZoomIn className="w-4 h-4" />
                                                <span>Klik untuk Perbesar</span>
                                            </div>
                                        </div>

                                        <div className="flex items-center justify-between w-full pt-2 px-1 text-[11px] text-slate-500 font-medium">
                                            <span className="flex items-center gap-1 font-bold text-slate-700 bg-white px-2.5 py-0.5 rounded-full border border-slate-200 text-[10px]">
                                                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                                                Scan m-Banking / E-Wallet
                                            </span>
                                            <span className="text-[10px] text-terracotta font-bold flex items-center gap-1 group-hover:underline">
                                                <Maximize2 className="w-3 h-3" /> Perbesar
                                            </span>
                                        </div>
                                    </div>

                                    {/* Quick Actions Bar (Perbesar & Unduh) */}
                                    <div className="flex items-center justify-center gap-2 max-w-[360px] mx-auto">
                                        <button
                                            type="button"
                                            onClick={() => setIsQrZoomOpen(true)}
                                            className="flex-1 py-2 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all border border-slate-200 cursor-pointer"
                                        >
                                            <ZoomIn className="w-3.5 h-3.5 text-terracotta" />
                                            <span>Perbesar</span>
                                        </button>
                                        <button
                                            type="button"
            ...
            {/* Lightbox Modal: Perbesar Tampilan Kode QRIS Resolusi Penuh */}
            {isQrZoomOpen && businessInfo?.qris_image_url && (
                <div
                    className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200"
                    onClick={() => setIsQrZoomOpen(false)}
                >
                    <div
                        className="relative bg-white rounded-3xl p-5 sm:p-6 max-w-lg w-full shadow-2xl border border-slate-200 text-center space-y-4 animate-in zoom-in-95 duration-200"
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="text-left">
                                    <h4 className="text-xs font-bold text-slate-900 line-clamp-1">{storeTitle}</h4>
                                    <p className="text-[10px] text-slate-400">Pindai kode QRIS toko untuk pembayaran</p>
                                </div>
                            </div>
                            <button
                                type="button"
                        ...
                        {/* Gambar QRIS Ukuran Besar */}
                        <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200 flex items-center justify-center overflow-hidden">
                            <img
                                src={businessInfo.qris_image_url}
                                alt="Kode QRIS Toko Resolusi Penuh"
                                className="max-h-[60vh] w-auto max-w-full object-contain rounded-xl bg-white p-2 border border-slate-200 shadow-sm"
                            />
                        </div>

                        {/* Total Tagihan & Info */}
                        <div className="bg-amber-50 p-3 rounded-2xl border border-amber-200 text-center space-y-0.5">
                            <p className="text-[11px] text-amber-900 font-semibold">Total Tagihan:</p>
                        ...
                        <div className="flex items-center justify-center gap-3 pt-1">
                            <button
                                type="button"
                                onClick={handleDownloadQris}
                                className="flex-1 py-2.5 px-4 bg-terracotta hover:bg-terracotta-hover text-white rounded-xl text-xs font-bold shadow-md shadow-terracotta/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
                            >
                            ...
                            <button
                                type="button"
                                onClick={() => setIsQrZoomOpen(false)}
                                className="py-2.5 px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold border border-slate-300 transition-all cursor-pointer"
                            >
                                Tutup
                            </button>
                        </div>
